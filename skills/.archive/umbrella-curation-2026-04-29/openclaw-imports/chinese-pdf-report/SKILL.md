...
- [ ] No local workstation path such as `/Users/<name>` or `/Users/toby` leaked from source content, tables, captions, or footer metadata
- [ ] No raw Markdown artifacts such as `####` heading markers leaked into the final PDF
...
## Important pitfall: `page-break-before: always` on heading elements creates blank separator pages

Observed in real usage when generating a multi-section consulting-style Chinese PDF from HTML:

- The CSS included `h1 { page-break-before: always; }` to force each major section to start on a new page
- Chrome headless respected the break BEFORE each h1 even when the previous page was not full
- Result: a blank page appeared before each h1 section — 3 blank pages in a 12-page document
- PyPDF2 extraction showed 0 chars on those blank pages
- The fix was to remove `page-break-before: always` from the CSS and instead use: (a) `page-break-after: always` on the cover, and (b) explicit `<div class="page-break">` elements only where truly needed (e.g., before heavy table sections)

Guideline:

- Do NOT put `page-break-before: always` on `h1` or other heading selectors in a Chrome-headless PDF workflow
- The one exception is `h1:first-of-type { page-break-before: avoid; }` — but even that can cause confusion
- Preferred approach: use `page-break-after: always` on explicit "page break before this" wrappers, not on structural heading tags
- After export, verify each page has content (PyPDF2 `extract_text()`) — blank pages are silent failures
...
## Important pitfall: Gateway/session interruption can leave “delivered” PDFs absent on disk

Observed while re-exporting a YouTube transcript report as a Chinese PDF after a gateway restart:

- a previous assistant response claimed a PDF had been delivered, but the target directory contained only `.DS_Store`
- the correct recovery was **not** to trust the prior message or invent the path; it was to re-discover the task directory, list actual files, and regenerate from subtitles/source material
- after regeneration, the PDF was verified with `PyPDF2` for page count, extractable text, blank pages, and local path leakage, then visually checked through a QuickLook thumbnail

Guideline:

1. After any gateway interruption, context compaction, or prior-turn uncertainty, verify the artifact exists before saying it is done.
2. Use a concrete file search/listing in the expected output directory, not memory of a previous response.
3. If only `.DS_Store` or scaffolding directories exist, treat the artifact as missing and regenerate it.
4. Never present a path from a prior natural-language response as confirmed unless the filesystem verifies it.
5. Keep the final report concise: state the regenerated PDF path and the QA checks passed.

## Important pitfall: Clean YouTube VTT from the raw file, not from numbered `read_file` output

Observed while producing a Chinese dialogue article from YouTube captions:

- `read_file()` returned display text with line numbers/pipes, e.g. `1|WEBVTT`, `9|...`
- processing that string as if it were raw VTT polluted the transcript with cue numbers and `WEBVTT` metadata
- the corrected workflow was to read the subtitle file directly from disk in Python with `Path(...).read_text(...)`, strip VTT headers/timecodes/cue IDs, then de-overlap rolling captions and normalize known proper nouns

Guideline:

```python
from pathlib import Path
raw = Path('/path/source.zh-CN.vtt').read_text(encoding='utf-8', errors='ignore')
# strip WEBVTT headers, timecode lines containing '-->', numeric cue IDs,
# HTML tags, duplicate rolling-caption fragments, then normalize names/terms
```

Do not use line-numbered tool display output as the cleaning source for final transcript/PDF content.
