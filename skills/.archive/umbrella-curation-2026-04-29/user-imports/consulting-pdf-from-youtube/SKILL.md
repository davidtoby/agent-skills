...
### Step 1c: Local file branch — no yt-dlp, direct Whisper

Use this branch when the source is a **local video file** (not a URL). There is no yt-dlp, no subtitle discovery, and no quality gate for auto-subs — go straight to ffmpeg audio extraction + Whisper.

**1c.1 — Get metadata from filename and public knowledge**

For local files, yt-dlp metadata is unavailable. Derive metadata from:
- The file name (often contains speaker, topic, date, venue)
- Public knowledge about the speaker/institution (e.g., Li Lu at Himalaya Capital, Peking University speech)
- The video's context (upload date, source channel if known)
- Write a `video_metadata.json` with whatever can be inferred
...
meta = {
    ...
    "duration_s": 10158,  # from ffprobe
    "file_path": "/path/to/video.mp4",
}
```

Use `ffprobe` to get duration, codec info, resolution:
```bash
...
```bash
ffmpeg -y -i "/path/to/video.mp4" -vn -ac 1 -ar 16000 "$WORKDIR/audio_16k.wav"
```

- Always extract to 16kHz mono WAV (Whisper's native format)
- Duration check: for a 2h49min video, expect ~310MB WAV file

**1c.3 — Transcribe with faster-whisper**

No subtitle quality gate to run — go directly to Whisper.

Model size strategy for **local files** (different from YouTube because there's no subtitle shortcut):

| Content type | Model | Rationale |
|---|---|---|
| Short <30min, clear speech | `base` or `small` | Fast, accurate enough |
| 30min–2hr, clear speech | `small` | Good balance (~2–3× real-time on CPU) |
| 2hr+, clear formal speech (lecture, talk) | `small` | 2h49min takes ~25min with `small`; `medium` would take 45-60min+ |
| 2hr+, heavy proper nouns (names, places, terms) | `medium` | Better accuracy on named entities |
| Any length, noisy recording | `medium` or `large-v3` | VAD filter helps but bigger model needed |

**Key insight from real usage:** For a 2h49min clear Chinese lecture, `small` model produced 3,757 segments with 36,132 characters and 100% language confidence (zh). The transcript was clean enough for professional-level analysis. `medium` would add 30-40 extra minutes of processing time for marginal quality gain.

**⚠️ Execution method**: Do NOT use `execute_code` for Whisper transcription — it has a ~300s hard timeout. Use **background terminal with `notify_on_complete=true`**. Write the transcription script to a file first:

```bash
cat > /tmp/whisper_transcribe.py << 'PYEOF'
import json, os
...
segments, info = model.transcribe(
    "<wav_file>",
    language="zh",           # force language when known
    beam_size=5,
    vad_filter=True,
...
**⚠️ Background wait clamp**: The `wait` action on background processes is clamped to 60s per call. For long transcriptions (>10 min), this means you'll need to re-issue `wait` calls or poll. The `notify_on_complete=true` flag will notify you when done — trust it and avoid manual polling.

**1c.4 — Convert Whisper output to cleaned transcript**

Skip the SRT cleaning (there's no SRT). Convert the Whisper JSON directly:
```python
import json
...
with open("transcript_clean.txt", "w") as f:
    for s in segments:
        ...
        f.write(f"[{h:02d}:{m:02d}:{sec:05.2f}] {s['text']}\n")
```

**1c.5 — Quality check Chinese ASR**

For Chinese content with `small` model, expect:
- Characters per minute: ~200–400 (lower than auto-subs due to longer segments)
- Known names/terms: generally correct for famous names (李录, 巴菲特, 芒格); verify in context
- Proper noun errors: less common with formal lecture audio than with noisy social media clips
- **Always spot-check**: read first 20 and last 10 transcript lines for obvious garbling before proceeding

Then continue with **Step 4** (the subagent analysis pipeline is identical).
...
- **Rolling/word-by-word auto-captions**: Some YouTube auto-captions use a cumulative-extension format where each consecutive entry shares >60% common-prefix overlap with the previous one, adding only 2–5 new words. These look like a single sentence being repeatedly extended rather than a triplicated block. For this format:
  - Use `os.path.commonprefix([prev_text, curr_text])` to detect overlap. If the common prefix is ≥50% of the minimum entry length, they belong to the same "rolling group."
  - Group consecutive entries this way; keep only the **last (most complete) entry** from each group.
  - Real-world data point: a 90-min English interview had ~5,150 raw entries → after rolling-group dedup → ~2,571 entries (28.5/min). Compare to the ~1,700 entries that triplicated format produces for a similar duration.
  - The rolling-group dedup algorithm is different from `[::3]` (which drops complete blocks) and from "keep all" (which retains every fragment). It is the correct choice when the common-prefix test shows >50% overlap for >80% of consecutive entry pairs. When in doubt, inspect the first 20 entries visually — rolling captions are immediately recognizable by their incremental word extension pattern.
...
For **religious / political / values-advice videos** (marriage, parenting, masculinity/femininity, family roles, civic morality), separate the report into three layers:
- `可验证事实`: speaker identity, organization, family/biographical facts, publication provenance.
- `可借鉴方法`: practical behaviors that can be evaluated outside the ideology (e.g., phone-free time, scheduled couple time, regular rituals).
- `价值/神学/意识形态判断`: claims such as “marriage is a covenant,” “husbands should lead spiritually,” or other role prescriptions. Do not present these as empirical findings.

Verification anchors for this class:
- Use neutral biographical sources first (Britannica, NPR/AP/Reuters, official organization pages) to confirm people, spouses, organizations, dates, and repost/compilation status.
- Use academic or peer-reviewed sources for empirical claims about marriage stability, religion, cohabitation, digital detox, or child outcomes. Label observational associations as such; do not convert them into causal claims.
- Treat claims sourced mainly to church blogs, advocacy groups, or partisan think tanks as `部分支持/需谨慎`, especially when the original study or methodology cannot be verified.
- Add a “视频未提及的关键背景” section for speaker controversy, political/religious incentives, source provenance, and applicability boundaries.
