...
name: lark-event
description: "Lark/Feishu real-time event listening, subscribing, and consuming. Streams events as NDJSON via lark-cli event consume with an EventKey argument, covering IM message receive, reactions, chat member changes, and similar scenarios. Use for Lark bots, real-time message processing, long-running subscribers, and streaming webhook/push handlers. Supports bounded runs via --max-events and --timeout plus a stderr ready-marker contract for AI-agent subprocess workflows."
metadata:
  requires:
    bins: ["lark-cli"]
