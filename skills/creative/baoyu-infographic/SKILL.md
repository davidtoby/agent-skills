...
Choose the rendering path based on text fidelity requirements:

#### A. Prompt-based image generation

Use the `image_generate` tool with the assembled prompt from Step 5 when the infographic is primarily visual, decorative, or low-text.
...
#### B. Deterministic local rendering for text-heavy infographics

When the user asks for a fact-heavy, bilingual/multilingual, educational, data-rich, or label-heavy infographic, prefer deterministic rendering with local tools instead of pure image generation. This avoids common image-model failures: misspelled labels, garbled Chinese, incorrect numbers, missing sections, and inconsistent bilingual text.

Recommended deterministic routes:
- **Pillow PNG canvas** for fixed-size posters, diagrams, bilingual labels, cards, legends, and simple shapes.
- **HTML/CSS → screenshot/PDF** for responsive or typography-heavy layouts.
- **SVG** for vector diagrams, arrows, icons, technical schematics, and scalable posters.

Deterministic rendering workflow:
1. Create a task-specific output folder under the user-preferred output root when known, e.g. `~/.Hermes/workspace/output/<topic-slug>/`.
2. Probe usable fonts before rendering. On macOS, check common Chinese-capable fonts such as `/System/Library/Fonts/STHeiti Medium.ttc`, `/System/Library/Fonts/STHeiti Light.ttc`, `/System/Library/Fonts/Supplemental/Arial Unicode.ttf`, or PingFang if present.
3. Write a reproducible generator script (`generate_<topic>_infographic.py`, `.html`, or `.svg`) alongside the final image.
4. Render at explicit high resolution when requested: e.g. 3840×2160 for 4K landscape, 2160×3840 for portrait, or 3000×3000 for square.
5. Verify with tools before delivery: open/read image metadata for dimensions and mode, check file size, and run `vision_analyze` when available to inspect readability and content completeness.
6. If QA finds missing labels, weak visual hierarchy, crowded bilingual text, or missing caveats/source notes, patch the generator and re-render before final delivery.
