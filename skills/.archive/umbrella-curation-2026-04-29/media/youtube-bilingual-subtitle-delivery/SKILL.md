...
### Direct JSON3 caption URL fallback from metadata

If `yt-dlp --write-auto-subs` fails with HTTP 429 but `yt-dlp --dump-single-json` succeeded, do not immediately fall back to Whisper. The metadata often contains usable `automatic_captions[lang]` track URLs. Fetch the `json3` URL directly for the primary language and parse it into a timing scaffold.

```python
import json, urllib.request, pathlib
m = json.load(open('video_metadata.json'))
for lang in ['en-orig', 'en', 'zh-Hans', 'zh-Hant']:
    tracks = (m.get('automatic_captions') or {}).get(lang) or []
    js = next((t for t in tracks if t.get('ext') == 'json3'), None)
    if not js:
        continue
    req = urllib.request.Request(js['url'], headers={'User-Agent': 'Mozilla/5.0'})
    try:
        data = urllib.request.urlopen(req, timeout=30).read()
        pathlib.Path(f'captions_{lang}.json3').write_bytes(data)
        print(lang, len(data))
    except Exception as e:
        print('caption fetch failed', lang, e)
```

Parse JSON3 `events[*].tStartMs`, `dDurationMs`, and `segs[*].utf8` into SRT events, then group tiny auto-caption events into readable chunks before translation. This is especially useful when English JSON3 downloads work but translated Chinese tracks still return 429.
...
- For music videos, audit caption text quality even when `--list-subs` shows `en-orig` or other auto tracks. Auto/ASR captions can contain badly mangled lyrics (for example repeated hooks rewritten as unrelated words). Treat them as disposable if lyrics are wrong.
- For lyric videos, use Whisper primarily as a timing scaffold, then manually correct the English against known/repeated lyrics before translating. Preserve repeated hooks consistently and regroup lines musically/readably instead of preserving tiny raw ASR fragments.
