...
- **Google News redirect returns `Error 400 (Bad Request)`** → first check whether the RSS article URL was accidentally truncated in printed output or by debug code such as `link[:160]`. Re-run an exact-title Google News RSS query and copy/use the full `<link>` value, including the final `?oc=5`, before calling `browser_navigate()`.
- **`content.js` returns `HTTP 401/Forbidden` on Reuters** → use exact-title Brave/search/RSS results as confirmation for the key fact pattern, then prefer an accessible primary source / AP / BBC / company blog for digest entries when you need fuller context. If using Reuters anyway, keep wording conservative and cite only the confirmed fact pattern plus the canonical Reuters URL resolved through Google News.
...
6. Verify the **canonical article page date/time**, not only the Google News RSS `pubDate`. Google News may resurface or cluster older canonical articles with a fresh RSS timestamp; discard items whose article metadata/body date falls outside the requested window unless the page clearly says it was updated or the user allows still-relevant older background.
7. For high-end Chinese/Feishu briefings, resolve each selected story to its publisher URL before writing. `urllib` on a Google News `/rss/articles/...` URL may only return the Google wrapper; `browser_navigate()` plus `browser_console({"expression":"location.href"})` is the reliable path even when the snapshot says “Empty page”.
