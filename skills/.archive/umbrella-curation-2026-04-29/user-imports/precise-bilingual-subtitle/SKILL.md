...
### Pitfall 0: Music/lyric clips can still be wrong with coarse Whisper segments
For songs, movie endings, lyric videos, worship clips, or any music-heavy segment, a normal Whisper SRT may produce coarse 10-30 second blocks, hallucinated pre-vocal captions, or repeated-hook wording errors. Do not treat those segments as final timing.

Repair pattern when the user says the subtitles do not match the picture/audio:
1. Re-run Whisper with word timestamps and inspect the first real sung/spoken words:
   ```bash
   ...
   ```
   If `medium` tries to re-download a large model and risks timeout, use an already available smaller model first; word-level timing from `small` is often enough to fix the sync.
2. Compare the master SRT against the word timestamps. Look specifically for too-early starts before vocals begin, long blocks spanning multiple lyric lines, and ASR mistakes such as repeated hooks being misheard (`Go West` → `Go ahead`).
3. Rebuild the SRT as short lyric/phrase events, using word starts/ends as anchors. A good music-video repair may turn 8-12 coarse events into 25-40 short events.
4. QA with targeted frames before and after the first vocal entry: e.g. one frame just before vocals should show no subtitle, one frame at first lyric should show the exact first line, one hook frame should show the matching hook text.
5. If the video enters a credit roll or dense on-screen text, keep a complete master SRT/softsub, but create a separate hardcode SRT variant that stops or moves subtitles before credits. Optionally deliver both a recommended non-obscuring hardcode and a full-lyrics hardcode.
