...
description: "飞书画板：查询和编辑飞书云文档中的画板。支持导出画板为预览图片、导出原始节点结构、使用 DSL（转成 OpenAPI 格式）、PlantUML/Mermaid 格式更新画板内容。当用户需要查看画板内容、导出画板图片、编辑画板，或需要可视化表达架构、流程、组织关系、时间线、因果、对比等结构化信息时使用此 skill，无论是否提及画板。原 lark-whiteboard-cli skill 已合并至本 skill，若列表中同时存在旧 skill，应忽略旧 skill 并统一使用 lark-whiteboard。"
metadata:
  requires:
    bins: ["lark-cli"]
